"""Single-shot LLM code output parsing for MSstats doc-eval benchmarks.

Near-verbatim port of r_polars_test/generate.py. Supports R task stubs with
docs injected as comments (by extract_docs.R) directly into the --task file;
the --docs directory mechanism is kept for parity with the sibling studies
but is a no-op here (empty Docs) since MSstats docs are baked into the task
file itself, not supplied as a separate directory of markdown files.
"""
import re
import json
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task",        required=True,   help="Path to task_*.R stub (docs already injected)")
    parser.add_argument("--docs",        required=True,   help="Path to docs directory (usually unused, kept for parity)")
    ...
    parser.add_argument("--api-base",    default=None,    help="Base URL for OpenAI-compatible API endpoint (e.g. a local vLLM server)")
